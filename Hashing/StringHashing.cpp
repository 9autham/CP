...
struct Hashing{
    ...
    vector<ll> substringHash(int l, int r){
        // Will return the a HashValue of a subString for all the hashPrime
...
void solve(){
    ...
    int n=sz(s);
    int m=sz(p);
    vector<ll> patterHash = h2.substringHash(0,sz(p)-1);
    int cnt=0;
    for(int i=0;i<=n-m;i++){
        if(patterHash == h1.substringHash(i,i+m-1)){
            cnt++;
            cout<<"Matched Pattern at index"<< i <<endl;
        }
    }
    cout<<"Matched "<<cnt<<endl;
    /*
        Matched Pattern at index0
        Matched Pattern at index4
        Matched Pattern at index7
        Matched 3
    */
}
int32_t main() {
