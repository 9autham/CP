...
Problem Statement : You have 2 operations to be done 
                        ...
                        2. Query the second - min from l to r

In this Sgt required storing of min and second min
...
vector<int>arr;
pair<int,int> mergeNodes(pair<int,int> left, pair<int,int> right){
    // {firstMinimun, secondMinimum}
    if(left.first>right.first){
        return {right.first, min(right.second,left.first)};
    }
    else{
        return {left.first, min(left.second,right.first)};
    }
}
void build(int index, int start,int end) {
    if(start == end) {
        // leaf node
        sgt[index]={arr[start],INF};
        return;
    }
    int mid = (start + end)/2;
...
void update(int index, int start, int end, int pos, int val){
    if(start==end){
        arr[pos]=val;
        sgt[index]={arr[pos],INF}; // Root node will not have second Minimum
        return;
    }
    int mid=(start+end)/2;
    if(pos<=mid){
...
pair<int,int> query(int index, int start, int end, int l, int r){
    if(start>=l && end<=r){
        ...
        return {INF,INF}; // Default value
    }
    else{
        int mid = (start+end)/2;
        pair<int,int> leftAns=query(2*index, start, mid,l,r);
        pair<int,int> rightAns=query(2*index+1, mid+1, end, l, r);
...
int32_t main() {
    ...
    while(q--){
        ...
        if(type==1){
            ...
            int l,r;
            cin>>l>>r;
            cout<<query(1,0,n-1,l,r).second<<endl;
        }
    }
    return 0;
}